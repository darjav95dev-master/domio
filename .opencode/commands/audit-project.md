...
1. Invoca `refactor-suggester` vía el tool `task`:
   `task({ subagent_type: "refactor-suggester", ... })`.
   Genera/actualiza `.specify/memory/refactor-backlog.md`.

2. Invoca `quality-reviewer` vía el tool `task` en modo "audit
   completo": en lugar de revisar un diff, revisa todo el código actual
   contra la constitución y `architecture.md`.

3. Verifica que los tests pasan globalmente (`pnpm verify` via `bash`).

4. Verifica la cobertura global y crítica (via `bash`).

5. Si el proyecto tiene contrato HTTP con otro repo, invoca
   `contract-guardian` (via `task`) para verificar sincronización.

6. Produce un **reporte ejecutivo** que combina los outputs anteriores
   en `.specify/memory/audit-report-<fecha>.md` (via `write`).
...
Para abordarlas, las incorporas al `.specify/memory/roadmap.md` con prefijo `R-` (de
