...
| Tabla                    | Campos clave                                                                                                                              | Notas                                                                              |
|--------------------------|-------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------|
| `promociones`            | `id`, `tenant_id`, `slug`, `name`, `kind`, `status`, `operation`, `property_type`, `construction_status`, `island`, `municipality`, `address`, `location` (PostGIS), `location_approx` (PostGIS), `map_privacy_mode`, `seo_title` (NULL), `seo_description` (NULL), `assigned_agent_id`, `draft_payload` (JSONB NULL), `created_at`, `updated_at` | `kind`: portfolio/external. `status`: DRAFT/PUBLISHED/RESERVED/SOLD/RENTED/WITHDRAWN. `operation`: SALE/RENT/SALE_AND_RENT. `property_type`: enum cerrado (piso, ático, casa, chalet, dúplex, estudio, local, oficina, nave, garaje, trastero, terreno). `construction_status`: ON_PLAN/IN_CONSTRUCTION/READY, NULL. Editable por operador; suele coexistir con `plazos_garantias.entrega_estimada` — el operador tiene la autoridad y el editor emite un warning suave si ambos divergen. `map_privacy_mode`: EXACT/AREA. `location_approx` es el centroide del municipio, se rellena siempre para permitir renderizar en modo `AREA`. `draft_payload` para autoguardado (se limpia al publicar). |
...
- `(tenant_id, construction_status)` en `promociones` — sirve al filtro público de "estado de obra" en el catálogo
...
19. **`construction_status` es autoritativo sobre `plazos_garantias.entrega_estimada` para el filtro público.** El campo lo controla el operador y el catálogo filtra por él directamente. Si la etiqueta contradice la fecha del bloque editorial, el editor emite un warning suave (no bloqueante) del tipo *"marcado como sobre plano pero la fecha de entrega es de hace 2 meses"*. El guardado se permite: la palabra final es del operador. Test verifica que el warning aparece cuando corresponde y que la persistencia nunca se bloquea por esta divergencia.
...
**Cambios desde v2.0:** añadida columna `construction_status` (`ON_PLAN`/`IN_CONSTRUCTION`/`READY`, NULL) en `promociones` — resuelve el filtro público "estado de obra" del catálogo con una consulta directa e indexable en lugar de derivarlo del bloque `plazos_garantias`. Índice compuesto `(tenant_id, construction_status)` en §6.6. Nueva regla técnicamente protegida §7.19: warning suave (no bloqueante) en el editor si la etiqueta contradice el `entrega_estimada` del bloque, respetando la autoridad del operador. Alineado con `roadmap.md` v1.1.
